...
export async function fetchTicketsModuleDataRequest(): Promise<{
  ...
  const [activeRes, sitesRes, localitiesRes] = await Promise.all([
  ...
  // Trash loading should never block initial active ticket rendering.
  const trashController = new AbortController();
  const trashTimeout = setTimeout(() => trashController.abort(), 1500);
  let trashRes: Response | null = null;
  try {
    trashRes = await fetch('/api/tickets/list?trash=true', {
      cache: 'no-store',
  ...
  return {
    ...
    activeData,
    trashOk: Boolean(trashRes?.ok),
    trashStatus: trashRes?.status ?? 0,
    trashData,
    sitesOk: sitesRes.ok,
    sitesData,
