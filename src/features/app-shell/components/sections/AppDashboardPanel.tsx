...
import { EXTERNAL_LINKS } from '@/features/app-shell/core/noc/noc-config';
import { getShiftColor, SHIFTS_DATA } from '@/features/app-shell/core/planning/shifts';
import { getIndividualRestAgent, getShiftScheduleForDate } from '@/features/app-shell/core/planning/planning-utils';
import type { Task, UserProfile } from '@/features/app-shell/core/shared/types';
...
export function AppDashboardPanel({ user, userRestInfo, tasks, allUsers, onRefresh }: AppDashboardPanelProps) {
  ...
  useEffect(() => {
    ...
    const handleConfigChange = () => loadDashboardConfig();
    window.addEventListener('storage', handleConfigChange);
    window.addEventListener('noc-dashboard-config-updated', handleConfigChange);
    return () => {
      window.removeEventListener('storage', handleConfigChange);
      window.removeEventListener('noc-dashboard-config-updated', handleConfigChange);
    };
  }, []);

  const visibleQuickCards = [
    dashboardConfig.showIncidentsWidget ? 'incidents' : null,
    dashboardConfig.showSlaWidget ? 'sla' : null,
  ].filter(Boolean);
  ...
  return (
    <motion.div key="dashboard" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-4">
      ...
      <div className={`grid gap-3 ${visibleQuickCards.length === 2 ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
        <Card className="p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">Taux presence</span>
        ...
        {dashboardConfig.showIncidentsWidget && (
          <Card className="p-3">
            <div className="flex items-center justify-between">
      ...
      <div className={`grid grid-cols-1 gap-4 ${showActivitySection && showShiftSection ? 'lg:grid-cols-2' : 'lg:grid-cols-1'}`}>
        {showActivitySection && (
          <Card>
            <CardHeader className="pb-2 pt-4">
        ...
        {showShiftSection && (
          <Card>
            <CardHeader className="pb-2 pt-4">
              <CardTitle className="text-base">Repartition par shift</CardTitle>
            </CardHeader>
            <CardContent className="pb-4">
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
