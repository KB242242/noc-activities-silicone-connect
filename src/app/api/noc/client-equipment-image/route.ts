...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'public', 'images', 'equipements');
    ...
    return NextResponse.json({
      ...
      fileName: file.name,
      mimeType: file.type,
      fileUrl: `/images/equipements/${fileName}`,
    });
  } catch (error) {
    console.error('NOC client-equipment-image upload error:', error);
    return NextResponse.json(
      { success: false, error: 'Impossible de televerser l\'image equipement.' },
      { status: 500 }
    );
  }
}
