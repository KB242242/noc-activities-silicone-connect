...
import { getShiftColor } from '@/features/app-shell/shifts';
import type { Conversation, InternalMessage, UserProfile } from '@/features/app-shell/types';
        ...
        <ScrollArea className="h-full">
          <nav className="p-3 space-y-1">
            ...
            {(user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') && (
              <Button variant={currentTab === 'admin' ? 'secondary' : 'ghost'} className={`w-full ${sidebarCollapsed ? 'lg:justify-center' : 'justify-start'} gap-3 h-10`} onClick={() => onSelectTab('admin')}>
