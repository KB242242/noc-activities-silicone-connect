...
import type { TaskCategory, TaskPriority, TaskStatus } from '@/features/app-shell/core/shared/types';

export const EXTERNAL_LINKS = [
  { id: '1', name: 'Suivi vehicules', url: 'https://za.mixtelematics.com/#/login', category: 'vehicles', icon: Truck, description: 'MixTelematics' },
  { id: '2', name: 'LibreNMS', url: 'http://192.168.2.25:2021/', category: 'monitoring', icon: Network, description: 'Monitoring reseau' },
...
export const TASK_STATUSES: Record<TaskStatus, { label: string; color: string; bgColor: string }> = {
  ...
  completed: { label: 'Terminee', color: 'text-green-600', bgColor: 'bg-green-100 dark:bg-green-900/30' },
  on_hold: { label: 'Suspendue', color: 'text-orange-600', bgColor: 'bg-orange-100 dark:bg-orange-900/30' },
  cancelled: { label: 'Annulee', color: 'text-slate-600', bgColor: 'bg-slate-100 dark:bg-slate-800' },
...
export const BADGE_CONFIG: Record<string, { label: string; icon: typeof Trophy; color: string }> = {
  ...
  needs_attention: { label: 'E Surveiller', icon: AlertTriangle, color: 'text-orange-500' },
};